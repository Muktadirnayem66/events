...
const ActionButton = ({eventId, interestedUserIds, goingUserIds,  formDetails}) => {
    ...
    const [isPending, startTransition]= useTransition()
     ...
     function toggleInterest(){
        if(auth){
             addInterestEvent(eventId, auth?.id)
    ...
    return (
        <div className={`w-full flex gap-4 mt-4 ${formDetails && "flex-1"}`}>
              ...
              <button disabled={auth && going} onClick={markGoing} className=" text-center w-full bg-[#464849] py-2 px-2 rounded-md border border-[#5F5F5F]/50 shadow-sm cursor-pointer hover:bg-[#3C3D3D] transition-colors active:translate-y-1">Going</button>
            </div>
    );
};
